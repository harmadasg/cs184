      ...
      if ((str.find_first_not_of(" \t\r\n") != string::npos) && (str[0] != '#')) {
        ...
        if (cmd == "light") { 
          if (numused == numLights) // No more Lights 
            ...
            if (validinput) {
              int stride =  4;
              matransform(transfstack, values + stride); // transform light positions
              for (i = 0; i < stride; i++) {
                lightposn[stride * numused + i] = values[i];
                lightcolor[stride * numused + i] = values[i + stride];
              } 
              ++numused; 
            }
          }
        ...
        // you can get a sense of how this works.  
          ...
          if (validinput) {
            mat4 translation_matrix = Transform::translate(values[0], values[1], values[2]);
            rightmultiply(translation_matrix, transfstack);
          }
        }
        else if (cmd == "scale") {
          validinput = readvals(s,3,values); 
          ...
          validinput = readvals(s,4,values); 
          if (validinput) {
            mat3 rotation_matrix = Transform::rotate(-values[3], vec3(values[0], values[1], values[2]));
